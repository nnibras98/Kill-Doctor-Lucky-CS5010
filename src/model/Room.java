...
public class Room {
  ...
  // There is a bug which I am trying to fix, not sure if it will make it to the
  // production tonight
  ...
  /**
   * Displays all the information of the room.
   *
   * @param allRooms List of rooms.
   * @param allItems List of items.
   * @param allHp    List of human players.
   * @param allCp       Computer player.
   */
  public void displayRoomInfo(List<Room> allRooms, List<Item> allItems, List<HumanPlayer> allHp,
      List<ComputerPlayer> allCp) {
    ...
    for (HumanPlayer player : allHp) {
      if (player.getCurrentRoom() == this.getIndex()) {
        System.out.println("- Human Player: " + player.getName());
      }
    }

    // Display Computer Players
    for (ComputerPlayer player : allCp) {
      if (player.getCurrentRoom() == this.getIndex()) {
